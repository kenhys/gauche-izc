...
CUT_EXPORT void test_renkan_sub2(void)
{
    cs_init();
    CSint **vint = cs_createCSintArray(ELEMENTS, 1, 3);
    ...
    /* B = | A - C | */
    cs_Eq(vint[1], cs_Abs(cs_Sub(vint[0], vint[2])));
    
    /*cs_findAll(vint, 3, cs_findFreeVar, found);*/
    cs_search(vint, ELEMENTS, cs_findFreeVarNbElements);

    /* A:2 B:1 C:3 */
    cut_assert_equal_int(2, cs_getValue(vint[0]));
    cut_assert_equal_int(1, cs_getValue(vint[1]));
